...
function values(mat) {
  let newMat = Object.fromEntries(mat)
  return newMat
}
console.log(values(matriz))
...
function pool (students, notes) {
  let resultado = []
  for (const alumno of students) {
    for (const calificacion of calificaciones) {
      if (alumno.id === calificacion.alumno_id) {
        resultado.push({ id: alumno.id, nombre: alumno.nombre, calificacion: calificacion.calificacion });
      }
    }
  }
  return resultado;
}
console.log(pool(alumnos, calificaciones))
...
function newPerson (pers){
  let newP = []
  for (i = 0; i < pers.length; i++ ) {
  newP.push([pers[i].nombre, pers[i].edad])
  }
  return newP
}

console.log(newPerson(personas))
